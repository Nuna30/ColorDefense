...
void UCreepWayGeneratorManager::CreateCreepWayGenerator(UWorld* InWorld, UBPActorPool* InBPActorPool, UVoxelGrid* InVoxelGrid, TArray<UCreepCheckPointGenerator*>& InCreepCheckPointGenerators, int32 InMaxRailCount, int32 InRailLength, TArray<FIntVector>& DirectionContainer, float InVoxelWidth, float InVoxelHeight)
{
    ...
    // 3. 데이터 주입 및 초기화
    NewCreepWayGenerator->Initialize(InWorld, InBPActorPool, InVoxelGrid, InCreepCheckPointGenerators, InMaxRailCount, InRailLength, DirectionContainer, InVoxelWidth, InVoxelHeight);
